...
export function createTask(payload: { name: string; durationDays: DurationDays }): { ok: boolean; reason?: string; task?: Task } {
  ...
  const task: Task = {
    ...
    questionnaire: { answers: [], totalScore: 0, routedModule: modulePrefix as RoutedModule },
    prompts: {},
    askFlow: {},
    renewHistory: [],
    listBadge: "聊天任务进行中",
...
export function submitQuestionnaire(taskId: string): { routed: RoutedModule; next: string } {
  ...
  if (score >= X) {
    t.questionnaire.routedModule = modulePrefix as RoutedModule; // 得分达标，后续走当前模块的问1/问2/问3流程
  } else {
  ...
  // 无论得分多少，都先进入"问1"流程
  return { routed: modulePrefix as RoutedModule, next: "问1" };
}
